...
class IdentityEncoder(nn.Module):
    def __init__(self, opt=opt):
        ...
        self.opt = opt
        self.add_module('block' + str(01), BasicBlock(3, 32, name="01", conv_std=0.025253814, kernel_size=7, stride=2, padding=3)) #(-1, 32, 128,128)
    ...
    def forward(self, x):
        x = x.view(-1, self.opt.image_channel_size, self.opt.image_size, self.opt.image_size)       # (-1, 3, 256, 256)
        net0 = self._modules['block' + str(01)](x)      # (-1, 32, 128, 128)
        net1 = self.pool1(net0)                         
        for j1 in range(11, 15):
            net1 = self._modules['block' + str(j1)](net1)   # (-1, 64, 64, 64)
        net2 = self.pool2(net1)                       
        for j2 in range(21, 27):
            net2 = self._modules['block' + str(j2)](net2)   # (-1, 160, 32, 32)
        net3 = self.pool3(net2)                         
        for j3 in range(31, 35):
            net3 = self._modules['block' + str(j3)](net3)   # (-1, 320, 16, 16)
        net4 = self.pool4(net3)                        
        for j4 in range(41, 43):
            net4 = self._modules['block' + str(j4)](net4)    # (-1, 256, 8, 8)

        net = self.avgpool(net4)                        # (-1, 256, 1, 1)
        net = net.view(-1, 256)         # (-1, 256)
